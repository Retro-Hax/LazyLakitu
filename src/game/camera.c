...
void lakitu_zoom(f32 rangeDist, s16 rangePitch) {
    ...
    } else if (gCameraMovementFlags & CAM_MOVE_FIX_IN_PLACE) {
        if ((sLakituDist += 30) > rangeDist) {
            sLakituDist = rangeDist;
        }
    } else {
    ...
    if (gCurrLevelArea == AREA_SSL_PYRAMID && gCamera->mode == CAM_MOVE_FIX_IN_PLACE) {
        rangePitch /= 2;
    }

    if (gCameraMovementFlags & CAM_MOVE_FIX_IN_PLACE) {
...
void update_camera(struct Camera *c) {
    ...
    if (c->cutscene == 0) {
        ...
        if (cam_select_alt_mode(0) == CAM_SELECTION_FIXED) {
    ...
    if (gCurrLevelNum != LEVEL_CASTLE) {
        ...
        // trigger is held
        if ((c->cutscene == 0 &&
            (gPlayer1Controller) && cam_select_alt_mode(0) == CAM_SELECTION_FIXED) /*Change 1:*/ //Camera is now locked when in Fixed Mario Cam Mode
...
s16 camera_course_processing(struct Camera *c) {
    ...
    if (!(sStatusFlags & CAM_FLAG_BLOCK_AREA_PROCESSING)) {
        switch (gCurrLevelArea) {
            case AREA_WF:
                if (sMarioCamState->action == ACT_RIDING_HOOT) {
                    cam_select_alt_mode(1) == CAM_SELECTION_FIXED;
                } else {
                    switch (sMarioGeometry.currFloorType) {
                        case SURFACE_CAMERA_8_DIR:
                            cam_select_alt_mode(1) == CAM_SELECTION_FIXED;
